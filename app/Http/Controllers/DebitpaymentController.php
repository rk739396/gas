...
class DebitpaymentController extends Controller
{
    public function index(Request $request)
    {
        ...
        // $data['company'] = DB::table('companies')->where('distributor_id', $distributorid)->orderby('id')->get();
        ...
        ->where('company_access_requests.user_id', $data['loginUser'])
    ...
    public function debit_payment(Request $request)
    {
        ...
        if (!$balance) {
            return back()->with('status', 'Not enough balance for debit.');
        }        
        if(floatval($balance->total_balance) >= floatval($request->input('amount'))){
        $debit->user_id = $loginUser;
        $debit->topup_id = $topup_id;
        $debit->distributor_id = $distributorid;
        $debit->sup_dist_id = $supdistid;
        $debit->fos_id = $fosid;
    ...
    public function view_pending(Request $request){
        ...
        if($loginRole == '4'){
            $data['debit'] = DB::table('topups')
            ->where('topup_type', '2')
            ->where('distributor_id', $distributorid)
            ->where('fos_id', $loginUser)
            ->where('payment_mode', 'cash')
             ->where(function ($query) {
        $query->where('payment_collect', '=', '0')
    ...
    public function collect_update(Request $request, $id)
    {
        ...
        $distributorid = $request->session()->get('logindistributor');
        $debit = Topup::find($id);
        $company = DB::table('companybalances')->where('retailer_id', $debit->user_id)->first();        
        $post_balance = $company->amount;
        ...
        //   DB::table('companybalances')->where('company_id',$debit->company_id)->where('retailer_id',$debit->retailer_id)->where('distributor_id',$distributorid)->update(['amount' => $tcb]);
          DB::table('companybalances')->where('company_id',$debit->company_id)->where('retailer_id',$debit->user_id)->where('distributor_id',$distributorid)->update(['amount' => $tcb]);
          }
